...
out_of_scope_internal : 직무 실무·기술·조직 판단·대인관계 등 사수님이 답할 수 있는 업무 관련 질문
  ...
  ⚠️ "사수가 시켰는데 어떻게 해?", "팀장님이 하라고 했는데 방법이?" 처럼 사수·팀장이 언급되더라도 업무 처리 방법이 핵심인 질문은 out_of_scope_internal 아님 → 반드시 rag로 분류
out_of_scope_external : 회사와 완전히 무관한 외부 주제 — 사수님도 답하기 어려운 것
  - 개인 재무·투자·커리어 (예: "투자 조언", "주식 추천", "커리어 상담 해줘", "이직 조언 해줘", "우리 회사 주식 살 수 있어요?", "회사 주식 매입") — ⚠️ 최저임금·퇴직금·연차·육아휴직·임금 등 노동법 관련은 절대 out_of_scope 아님, 반드시 rag로 분류
...
rag          : 회사 규정·문서·절차 질문 + 담당자·팀장 질문
  ...
  - 복지혜택 문의: "헬스케어 앱 구독 지원해줘", "건강관리 앱 구독료 지원받을 수 있어요?", "임직원 의료비 지원 어떻게 돼?", "복지 혜택 뭐 있어?" 등 회사가 제공하는 복지 관련 질문 → 반드시 rag
  ⚠️ 아래 구어체 표현도 전부 회사 복지 지원 문의이므로 반드시 rag로 분류 (맛집·음식 추천과 절대 혼동 금지):
    - 식대: "밥값 나와?", "점심값 줘?", "저녁값 지원돼?", "밥 사줘?", "야근하면 뭐 줘?", "카페에서 써도 돼?"
    - 출산·육아: "애 낳으면 뭐 줘?", "임신하면 혜택 있어?", "아기 생기면?", "둘째 낳으면?", "출산하면 뭐 해줘?"
    - 경조: "결혼하면 뭐 해줘?", "부모님 돌아가시면?", "경조금 얼마야?"
    - 자기계발: "책 사면 돈 줘?", "자격증 따면 지원해줘?", "강의 들으면 돈 줘?"
    - 의료: "병원 가면 돈 돌려줘?", "아프면 지원돼?", "치료비 나와?"
  - 근무 세부 규정: "코어타임이 언제예요?", "코어타임이 언제야?", "유연근무제 신청 방법", "재택근무 조건" 등 회사 내부 규정 문서에 명시된 근무 관련 질문 → 반드시 rag
  - 회사 IT 환경·도구: MFA 설정, VPN 사용법, Slack 채널 초대, Notion 권한, 비밀번호 변경, 계정 세팅, 소프트웨어 설치, 화면 잠금, 피싱 메일 대응, A4 용지 위치 등 사내 규정·환경 관련 질문
  - 주차·시설 관련: "주차 어디서 해", "주차장 위치", "주차 신청 방법", "주차비 얼마야", "회사 주차는" 등 사내 주차·시설 관련 질문 → 반드시 rag
...
communication: 아래 패턴 중 하나라도 해당되면 communication으로 분류
  ...
  - 업무 메일·메시지 작성 요청 (예: "메일 써줘", "이메일 작성해줘", "메시지 써줘", "메일 초안 써줘", "초안 작성해줘", "메일 어떻게 써야해?", "메일 어떻게 쓰지?", "연차 신청 메일 써줘", "재택근무 신청 메일 써줘", "재택근무 쓰고 싶은데 메일 어떻게 써야해?", "재택근무 신청하고 싶은데 메일 써줘", "경비 정산 메일 써줘", "사직서 메일 써줘")
  ⚠️ 메시지에 "메일", "이메일", "메시지 써", "초안" 중 하나라도 있으면 앞에 어떤 주제(재택근무·연차·경비 등)가 붙어 있어도 반드시 communication으로 분류. rag로 분류 금지.
  - 문서·보고서 작성 요청 (예: "보고서 템플릿", "문서 상신 어떻게 써", "업무 보고 형식", "주간 보고 어떻게 써")
...
preboarding  : 환영 레터·팀 소개 카드 명시적 요청에만 해당 (예: "환영 레터 보여줘", "팀 소개해줘")
  ...
  ⚠️ "첫날 준비 사항", "출근 전 준비", "첫날 일정", "서류 뭐 챙겨야", "출근 시간" 등 구체적 정보 질문은 preboarding이 아닌 반드시 rag로 분류
...
# ── Intent + Clarifying 통합 프롬프트 (첫 질문 전용, LLM 1회로 처리) ──

_INTENT_CLARIFYING_PROMPT = ChatPromptTemplate.from_messages([
    ("system", """사용자 메시지를 분석하여 JSON으로만 반환하세요.

## 1. intent 분류 (반드시 아래 중 하나)
chitchat     : 인사말, 잡담, 감정 표현, 날짜·입사일·근무일수 질문, 실수·걱정 표현, 온보딩 맥락의 일상 감정 표현(첫날 긴장·점심 부담·팀 분위기 걱정 등), 이전 답변에 대한 확인·출처 질문 (예: "이거 확실해요?", "어느 문서로부터 받은 기준이에요?", "출처가 뭐예요?", "근거가 뭐예요?", "맞아요?", "어디서 나온 정보예요?")
...
company_info : 점심시간·급여일·기본 출퇴근 시간·사무실 주소 등 기본 회사 생활 정보
  예) "출근 시간이 어떻게 돼?", "몇 시에 출근해야 해?", "퇴근 시간이 언제야?", "코어타임이 언제야?", "점심시간 몇 시야?", "급여일이 언제야?"
rag          : 회사 규정·문서·복지·IT 환경·법률·주차·시설 질문 (기본값, 노동법·근로기준법 포함)
  예) "A4 용지는 어디 있어요?", "Slack에서 확인했다는 표시는 어떻게 해요?", "소프트웨어 설치하고 싶은데 어떻게 해요?", "주차 어디서 해요?", "주차비 얼마예요?", "주차장 어디 있어요?", "입사 첫날 점심은 어떻게 하나요?", "첫날 점심은 어떻게 해요?", "헬스케어 앱 구독 지원해줘", "건강관리 앱 구독료 지원받을 수 있어요?", "임직원 의료비 지원 어떻게 돼?", "보안 USB 누구한테 받아?", "보안 USB 누구 한테 받아?", "장비 누구한테 받아?", "첫날 준비 사항", "출근 전에 미리 해야 할 일", "첫 출근 시간은 언제예요?", "회사 위치 알려줘", "서류 뭐 챙겨야 해요?", "첫날 일정이 어떻게 돼요?"

판단 우선순위: chitchat > out_of_scope_internal > out_of_scope_external > communication > company_info > preboarding > rag

## 2. clarifying 판단 (intent == "rag"일 때만)
모호한 질문(세부 의도가 2가지 이상 해석 가능)이면 clarifying 질문을 생성하세요.
명확한 질문이면 null을 반환하세요.

[명확한 질문 → null 반환]
- "~있어요?", "~있나요?", "~돼요?", "~되나요?" 형태는 모두 명확
- "야근하면 밥값 지원되나요?", "병원비 보험 처리 돼요?", "책 사면 회사에서 돈 돌려줘요?"
- "노트북 말고 모니터도 받을 수 있어요?", "사내 동호회 있어요?", "육아휴직 제도 있어요?"
- "연차 신청 방법 알려줘", "재직증명서 발급 어떻게 해요?", "복지카드 언제부터 써요?"
- "수습 기간이 얼마나 돼요?", "연차는 언제부터 생겨요?", "출퇴근 기록은 어떻게 해요?"
- "결혼하면 회사에서 뭐 해줘요?" → 경조사 지원 전체로 명확
- "반차", "반차요", "반차 알려줘", "반차가 뭐예요?" → 반차 제도 전반 안내 요청으로 명확 (clarifying 절대 금지)
- "경조사비", "경조사비요", "경조사비 알려줘" → 경조사비 지원 전반 안내 요청으로 명확 (clarifying 절대 금지)
- "수습평가", "연차", "육아휴직" 등 HR 제도 단어 단독 → 해당 주제 전반 안내 요청으로 명확
- "경비 정산은 어떻게 해요?", "법인카드 사용 후 어떻게 처리해요?" → 처리 절차 질문으로 명확 (clarifying 절대 금지)
- "소프트웨어 설치하고 싶은데 어떻게 해요?", "A4 용지는 어디 있어요?" → 구체적 질문으로 명확 (clarifying 절대 금지)
- "점심 영수증 제출해야 해?", "점심값 어떻게 정산해?" → 점심·식대 정산 절차 질문으로 명확 (clarifying 절대 금지)
- "생일에 혜택 있어?", "생일 혜택 알려줘" → 생일 복지 전반 안내 요청으로 명확 (clarifying 절대 금지)
- "제출 서류 확인하기", "제출 서류 알려줘", "입사 서류 뭐가 필요해요?" → 입사 서류 안내 요청으로 명확 (clarifying 절대 금지)
- "첫날 일정 확인하기", "첫날 어떻게 진행돼요?", "입사 첫날 순서 알려줘" → 첫날 일정·순서 안내 요청으로 명확 (clarifying 절대 금지)
- "담당자 문의처 확인하기", "담당자 연락처 알려줘", "누구한테 물어봐요?" → 담당자 연락처 안내 요청으로 명확 (clarifying 절대 금지)

[모호한 질문 → clarifying 질문 생성]
- "증명서 필요해요" → 재직/경력/원천징수/영문 중 불명확
- "정산이요" → 경비/식대/야근식대/출장비 중 불명확
- "결재 어떻게 올려요?" → 연차/경비/비품 중 불명확

clarifying 질문 작성 규칙: 존댓말, ①②③ 선택지 제시, "어떤 부분이 궁금하신가요?" 형식, ①②③④ 뒤에 반드시 공백 1칸 포함 (예: "① 신청 방법")

## 출력 형식 (JSON만, 다른 텍스트 절대 없음)
{{"intent": "rag", "clarifying": null}}
{{"intent": "rag", "clarifying": "어떤 부분이 궁금하신가요? ①신청 방법 ②발생 기준 ③잔여 확인"}}
{{"intent": "chitchat", "clarifying": null}}"""),
    ("human", "{message}"),
])

_intent_clarifying_chain = None


def _get_intent_clarifying_chain():
    global _intent_clarifying_chain
    if _intent_clarifying_chain is None:
        from core.llm import get_combined_llm
        _intent_clarifying_chain = _INTENT_CLARIFYING_PROMPT | get_combined_llm() | StrOutputParser()
    return _intent_clarifying_chain
...
아래 회사 정보를 바탕으로 친절하게 답변하세요. "안녕하세요" 등 인사말 없이 바로 답변으로 시작하세요. 반드시 ~이에요·~예요 말투로만 답변하세요. ~입니다·~습니다·"거랍니다"·"답니다"·"랍니다"로 끝나는 문장 절대 금지. "꼬박꼬박"·"착착" 같은 속어도 사용하지 마세요.
...
⚠️ 사용자가 "응원해줘", "힘내줘", "위로해줘" 등을 요청하면 사용자를 응원·위로하세요. AI 자신이 응원받은 것처럼 "감사해요" 등으로 답하지 마세요.
...
⚠️ 사용자가 다른 날짜를 제시하더라도 위 [오늘 날짜]를 절대 신뢰하세요. 사용자 말을 따라 날짜를 수정하거나 사과하지 마세요. 날짜 반박 시 "제가 받은 날짜 정보 기준으로는 오늘이 {today_date}이에요 😊" 한 문장으로만 정정하세요. 유저를 가르치거나 "혹시 ~라고 생각하셨나요?" 같은 표현 절대 금지.
{hire_info}
...
_OUT_OF_SCOPE_EXTERNAL_MESSAGE = (
    "저는 회사 규정·복지 전문 버디라서 그 부분은 도움드리기 어려워요. 😅 "
    "그런 건 팀 동료분들이 훨씬 잘 알고 계실 거예요!\n\n"
    "사내 규정·복지 관련 궁금한 건 언제든 물어봐 주세요. 제가 제일 잘하는 영역이거든요!"
)
...
_COMPANY_BENEFIT_KEYWORDS = [
    "밥값", "점심값", "저녁값", "식대", "식비", "복지포인트", "경조금", "자기계발비", "자기계발",
