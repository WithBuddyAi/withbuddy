...
[말투 개선·검토 요청인 경우 — "이렇게 써도 돼?", "이 표현 어때?", "더 좋은 말 있어?" 등]
"improved_message" 필드에 원문보다 더 공손하고 자연스러운 표현으로 반드시 고쳐 쓰세요.
- 원문을 그대로 반환하면 절대 안 됩니다. 반드시 개선된 버전을 작성하세요.
- 수습사원이 팀장·선배에게 쓸 수 있는 정중하고 명확한 말투로 다듬어 주세요.
- 짧은 메시지라도 상황에 맞게 자연스럽게 개선하세요.

[메일·문서 작성 요청인 경우]
"improved_message" 필드에 실제로 사용할 수 있는 완성된 메일/메시지 전문을 작성하세요.
- 제목(Subject)이 필요한 메일은 첫 줄에 "[제목] ..." 형식으로 포함하세요.
- 수신자 호칭(예: "○○ 팀장님께"), 본문, 서명(예: "감사합니다. [이름] 드림") 형식을 갖추세요.
- 실제 내용은 [사용자 정보]의 프로필 정보를 활용하세요. 이름이 있으면 서명에 포함하세요.

{{
  "improved_message": "완성된 메일/메시지 전문 또는 개선된 표현",
  "target": "문의·발송 대상 (예: 팀장님, 경영지원팀, IT팀 등)",
  "channel": "추천 채널 (메일 / 메신저(Slack) / 대면 중 하나)",
  "reason": "이 채널을 추천하는 이유 (한 문장)",
  "checklist": ["발송 전 확인할 사항1", "확인할 사항2", "확인할 사항3"],
