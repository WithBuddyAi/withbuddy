...
────────────────────────────────────────────
BE가 unanswered_question_logs에서 질문 + 임베딩을 꺼내
배치로 전송하면 AI가 군집화 + 요약을 생성해 반환한다.
...
def _greedy_cluster(items: List[ClusterItem], threshold: float) -> List[List[ClusterItem]]:
...
def _pick_representative(cluster: List[ClusterItem]) -> str:
    freq: dict = {}
    for item in cluster:
...
async def cluster_no_result_questions(req: ClusterRequest) -> ClusterResponse:
    ...
    raw = _greedy_cluster(req.items, _DEFAULT_THRESHOLD)
