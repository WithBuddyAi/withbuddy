...
public interface ChatMessageControllerDocs {
    ...
    @Operation(
        summary = "AI 답변 처리 상태 조회 (폴링)",
        description = """
            [목적] 비동기 AI 처리 중인 질문의 처리 상태와 완료된 답변을 조회한다.
            [베네핏] AI 응답이 5초를 초과할 때 클라이언트가 블로킹 없이 즉시 PENDING을 받고, \
            이후 폴링으로 완료 여부를 확인한다. 긴 답변 생성 시에도 대화가 중단되지 않고 \
            최대 55초까지 안정적으로 처리된다."""
    )
    @ApiResponses(value = {
