package com.withbuddy.buddy.onboarding.docs;
...
public interface OnboardingSuggestionControllerDocs {
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "온보딩 제안 목록 반환",
                    content = @Content(schema = @Schema(implementation = OnboardingSuggestionExposureResponse.class))),
            @ApiResponse(responseCode = "401", description = "인증 실패 — 유효하지 않은 토큰",
                    content = @Content(schema = @Schema(implementation = ErrorResponse.class)))
    })
    ResponseEntity<OnboardingSuggestionExposureResponse> exposeMyOnboardingSuggestion(
