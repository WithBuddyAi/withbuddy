...
function MessageList({
  ...
  // User Class 정리
  const userClass = `rounded-tl-[24px]
  rounded-tr-[4px]
  lg:rounded-tr-[8px]
  rounded-bl-[24px]
  rounded-br-[24px]
  ...
  return (
    <div>
      {messageList.map((message, index) => {
        ...
        const prevDate =
          index > 0 ? messageList[index - 1].createdAt?.slice(0, 10) : null;
            ...
            {isNewDate && (
              <div className="flex items-center justify-center">
                <p className="border-[1px] border-[#DEE2E6] bg-[#FFFFFF] w-[130px] md:w-[150px] py-[6px] px-[16px] rounded-[9999px] drop-shadow-sm text-[#495057] text-[12px] md:text-[14px] text-center mt-[16px]">
                  {currentDate &&
                    format(new Date(currentDate), "yyyy년 M월 d일", {
                      locale: ko,
                    })}
                </p>
              </div>
            ...
            <div
              className={
                message.senderType === "USER"
                  ? "flex justify-end"
                  : "flex justify-start items-start mt-[32px]"
              }
            >
              ...
              <div className="flex flex-col">
                <div
                  ...
                  style={
                    message.senderType === "USER"
                      ? {
                          background:
                            "linear-gradient(to right, #7DC1FF, #6BB5F2, #57A7E4, #4F9CD7, #4791CA)",
                        }
                      : {}
                  }
                >
                  ...
                  message.messageType === "send_error" ? (
                    <div className="flex flex-col md:flex-row items-start md:items-center gap-[10px]">
                      <p className="text-[#495057] text-[13px] lg:text-[16px]">
                        {message.content}
                      </p>
                      <button
                        onClick={handleRetry}
                          ...
                          <div>
                            ...
                            <div className="inline-flex flex-col">
                              {message.documents.map((doc) => (
                                <div
                                  key={doc.documentId}
                                  className="items-start py-[10px] px-[16px] rounded-[8px] border-[1px] border-[#336B971A] bg-[#F7FBFF5A] h-auto mt-[8px]"
                                >
                                    ...
                                    {doc.file && (
                                      <div className="flex flex-wrap gap-[8px]">
                                        ...
                                        <button
                                          ...
                                          className="text-[11px] text-[#336B97] underline"
                                        >
                                          다운로드
                                        </button>
                                      </div>
                      ...
                      {message.recommendedContacts?.length > 0 && (
                        <div className="inline-flex flex-col mt-[8px]">
                          {message.recommendedContacts.map((contact) => (
                            <div
                              key={`${contact.department}-${contact.name}`}
                              className="flex items-center gap-[12px] py-[10px] px-[16px] rounded-[8px] border-[1px] border-[#336B9733] bg-[#F7FBFF] mt-[8px]"
                            >
                              {/* 전화기 아이콘 */}
                              ...
                              <div className="flex flex-col gap-[2px]">
                                <p className="text-[12px] text-[#336B97] font-medium">
                                  {contact.department} {contact.name}{" "}
                                  {contact.position}
                                </p>
                                {contact.connects.map((c) => (
                                  <div key={c.type}>
                                    <p className="text-[11px] text-[#868E96] font-medium">
                                      {c.type} | <span>{c.value}</span>
                                    </p>
                                  </div>
                                ))}
                              </div>
                ...
                {/* 시간 */}
                <div
                  className={`${message.senderType === "USER" ? "text-right md:mr-[48px]" : "text-left ml-[16px]"}`}
                >
                  <p className="text-[#868E96] text-[10px] md:text-[16px] mt-[8px]">
                    {message.createdAt &&
                      format(new Date(message.createdAt), "a h:mm", {
                        locale: ko,
                      })}
                  </p>
                </div>
