...
- 사용자/회사/대화 원본 데이터의 최종 저장 책임은 Backend의 OCI MySQL DB System 9.7.0에 있다.
- AI 서버는 Redis 캐시와 RabbitMQ 큐를 보조 계층으로 사용한다.
- Redis와 RabbitMQ는 각각 별도 `VM.Standard.E2.1.Micro` 인스턴스로 분리 운영한다.
- Backend는 blue/green `VM.Standard.A1.Flex (2 OCPU / 12GB RAM)` 2슬롯으로 운영한다.
- AI 서버가 DB에 직접 접근해야 하는 경우는 운영 점검/마이그레이션 등 제한된 시나리오로 한정한다.
...
- 2026-07-02: 운영 기준을 OCI MySQL DB System 9.7.0, Backend blue/green A1.Flex 2 OCPU / 12GB x2, Redis/RabbitMQ E2.1.Micro 분리 구성에 맞게 정리.
