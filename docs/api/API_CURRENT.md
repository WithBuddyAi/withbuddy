...
| `/api/v1/chat/messages/stream` | 질문 전송 및 AI 답변 스트리밍 | `ACTIVE_USER`, `SERVICE_ADMIN` |
| `/api/v1/chat/*` | MyBuddy 채팅 메시지 조회, 화면 진입 로그, 빠른 질문 조회/클릭 로그 등 | `ACTIVE_USER`, `INACTIVE_USER`, `SERVICE_ADMIN` |
| `/api/v1/onboarding-suggestions/*` | 온보딩 제안 노출 처리 | `ACTIVE_USER`, `INACTIVE_USER`, `SERVICE_ADMIN` |
...
| `/api/v1/chat/documents/{documentId}/download` | 채팅 답변으로 수신한 TEMPLATE 문서만 다운로드 URL 발급 | `ACTIVE_USER`, `INACTIVE_USER`, `SERVICE_ADMIN`, `ADMIN` |
| `/api/v1/documents/{documentId}/download` | 회사/공통 TEMPLATE 문서 ID 기준 다운로드 URL 발급 | `ACTIVE_USER`, `INACTIVE_USER`, `SERVICE_ADMIN`, `ADMIN` |
| `/api/v1/documents/{documentId}/file` | TEMPLATE 문서 다운로드 302 리다이렉트 (Location=외부 pre-signed URL) | `ACTIVE_USER`, `INACTIVE_USER`, `SERVICE_ADMIN`, `ADMIN` |
...
- `INACTIVE_USER`는 `ACTIVE_USER`와 동일한 사용자 기능을 사용할 수 있으나, 질문 전송 API(`POST /api/v1/chat/messages/stream`)는 호출할 수 없다.
- `SERVICE_ADMIN`은 제품 내부 지표 조회용 계정이다. `ACTIVE_USER`와 동일하게 MyBuddy 기능을 사용할 수 있으며, 추가로 Swagger, Postman, 내부 운영 도구 등을 통해 관리자 지표 API(`/api/v1/admin/metrics/*`)를 호출할 수 있다.
...
- 프론트엔드는 `user.role` 값을 기준으로 `ACTIVE_USER`, `INACTIVE_USER`, `SERVICE_ADMIN`은 MyBuddy 화면으로, `ADMIN`은 관리자 계정 페이지로 분기할 수 있다.
...
| `user.role` | String | Y | 사용자 역할. `ACTIVE_USER`, `INACTIVE_USER`, `ADMIN`, `SERVICE_ADMIN` 중 하나 |
...
|---|---|---|
| `ACTIVE_USER` | 활성 신입 사용자 | MyBuddy 채팅 화면(`/mybuddy`) |
| `INACTIVE_USER` | 비활성 신입 사용자. 질문 전송만 제한되고 나머지 사용자 기능은 활성 사용자와 동일하게 사용 가능 | MyBuddy 채팅 화면(`/mybuddy`) |
| `ADMIN` | 고객사 관리자. 신입 계정 생성 등 고객사 관리자 페이지 접근 가능 | 관리자 계정 페이지(`/admin`) |
| `SERVICE_ADMIN` | 제품 내부 지표 조회용 계정. `ACTIVE_USER`와 동일하게 MyBuddy 기능 사용 가능 | MyBuddy 채팅 화면(`/mybuddy`). 지표 조회는 별도 프론트 화면 없이 Swagger, Postman, 내부 운영 도구 등을 통해 API 직접 호출 |
...
| 값 | 설명 |
|---|---|
| `ACTIVE_USER` | 활성 신입 사용자. MyBuddy 채팅 기능을 사용한다. |
| `INACTIVE_USER` | 비활성 신입 사용자. `ACTIVE_USER`와 동일한 사용자 기능을 사용할 수 있으나 질문 전송 API(`POST /api/v1/chat/messages/stream`)는 호출할 수 없다. |
| `ADMIN` | 고객사 관리자. 관리자 계정 페이지에서 신입 계정 생성 및 문서 관리 API를 호출할 수 있다. |
| `SERVICE_ADMIN` | 제품 내부 지표 조회용 계정. 일반 `ACTIVE_USER`와 동일하게 MyBuddy 기능을 사용할 수 있으며, 추가로 관리자 지표 API(`/api/v1/admin/metrics/*`)를 호출할 수 있다. 고객사 관리자 계정 페이지 API와 문서 관리 API는 호출할 수 없다. |
...
- 신입 사용자(`ACTIVE_USER`, `INACTIVE_USER`)와 제품 내부 관리자(`SERVICE_ADMIN`)가 호출하면 `403 Forbidden`과 `ACCESS_DENIED` 에러 코드를 반환한다.
...
- 생성되는 신입 계정의 `role`은 항상 `ACTIVE_USER`로 저장한다.
...
| `role` | String | 생성된 사용자 역할. 신입 계정 생성 API에서는 항상 `ACTIVE_USER` |
...
- 목록 조회 대상은 `users.role`이 `ACTIVE_USER` 또는 `INACTIVE_USER`인 신입 사용자 계정이다.
...
- `isActive`는 신입 사용자의 활성 상태를 반환한다. `role = ACTIVE_USER`이면 `true`, `role = INACTIVE_USER`이면 `false`이다.
- 인증 오류와 세션 만료/무효화 처리 방식은 **5-3. 인증 오류 및 세션 만료/무효화 처리**를 따른다.
...
신입 사용자(`ACTIVE_USER`, `INACTIVE_USER`)와 제품 내부 관리자(`SERVICE_ADMIN`)는 문서 관리 API를 호출할 수 없다.
...
- 신입 사용자(`ACTIVE_USER`, `INACTIVE_USER`)와 고객사 관리자(`ADMIN`)는 호출할 수 없다.
...
- 집계 대상 사용자는 기본적으로 `users.role`이 `ACTIVE_USER` 또는 `INACTIVE_USER`인 신입 사용자로 한정한다.
