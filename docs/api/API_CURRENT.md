...
- `ADMIN`은 고객사 관리자 계정으로, `users.account_status = ACTIVE`인 경우에만 관리자 계정 페이지에서 신입 계정 생성·조회 및 문서 관리 API를 호출할 수 있다.
...
| `ADMIN` | 고객사 관리자. `users.account_status = ACTIVE`인 경우에만 관리자 계정 페이지에서 신입 계정 생성 및 문서 관리 API를 호출할 수 있다. |
