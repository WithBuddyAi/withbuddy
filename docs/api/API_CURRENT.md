...
| `/api/v1/documents/{documentId}/download` | 회사/공통 TEMPLATE 문서 ID 기준 다운로드 URL 발급 | `USER(accountStatus=ACTIVE 또는 READ_ONLY)`, `ADMIN`, `SERVICE_ADMIN` |
| `/api/v1/documents/{documentId}/file` | TEMPLATE 문서 다운로드 302 리다이렉트 (Location=외부 pre-signed URL) | `USER(accountStatus=ACTIVE 또는 READ_ONLY)`, `ADMIN`, `SERVICE_ADMIN` |
