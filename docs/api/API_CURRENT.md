...
- 파일 크기는 최대 20MB로 제한한다.
- 파일이 비어 있거나 크기 제한을 초과하면 `400 Bad Request`, `FILE_001`을 반환한다.
...
- **v2.0.0 (2026-05-18)**:
  ...
  - 문서 업로드 가능 확장자, 회사당 문서 수 300개 제한, 파일 크기 20MB 제한, 목록 조회 `size` 최대 300, 휴지통 삭제 시 선택 삭제 API 사용 규칙을 추가함.