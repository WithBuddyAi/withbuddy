...
### 문서 다운로드 흐름 변경
- `GET /api/v1/documents/{documentId}/file` 동작을 **파일 바이트 직접 반환(200)** 에서 **302 Redirect**로 명시 변경.
- `Location` 헤더에 외부 Object Storage pre-signed URL이 포함되며, 백엔드는 파일 바이트를 프록시하지 않음.
- 보안/캐시 헤더 명시:
...
- 응답 본문에 다음 필드 기준을 명시:
  - `downloadUrl`: 내부 경로(`/api/v1/documents/{id}/file?source=...`)
  - `expiresIn`: pre-signed URL 유효시간(현재 기본 30초)
  - `source`: `PRIMARY` 또는 `BACKUP`

### 운영 목적 명시
- 최우선 목표: **API 서버 부하 절감**(대용량 파일 스트리밍 제거, 클라이언트 직접 다운로드).
- 보안 목표: pre-signed URL은 본문에 직접 노출하지 않고 `/file` 호출 시점에만 발급/전달.
